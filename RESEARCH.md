...
## 2026-09-17 23:50 PT — literature and practitioner sweep with a two-agent debate (user: "우리랑 비슷하게 한 거 논문이나 레딧글 찾아서 향상시킬 방법 찾아내, 멀티에이전트로 서로 토론도 하고"); protocol v4.1; round 46 pre-registered before any result
...
**Round-41 re-score under v4.1 (2026-09-18 00:10 PT, 0 trials, `analyze_v4.py _w0 _w2 --affected-from 2025-10-01`):** `_w2` (filings out of the graph agents) vs `_w0`: paired +2.3 bp/d (t +1.48, Newey-West +1.76); six blocks [0, 0, 0, +0.3, +12.8, −2.7] → three ties excluded, 2 of 3 informative (need 2); the 220 affected days from 2025-10-01 in four blocks [+2.2, +14.9, +13.6, −4.8] → 3 of 4. PASS under the amended rule as well; the adoption stands on its own record.

### Round 46 zero-trial diagnostics (2026-09-18 00:20 PT, the `_c3` ledger = the live warm start, 2024-08..2026-08; `diag_round46.py` in the job's tmp)
...
### Round 46 results (2026-09-18 00:45 PT, VM, fixed replay, code 6768748, gate v4.1): nothing passes — the fitted weights beat the equal-weight blend by a wide margin, λ = 1000 loses, dropping the direction-only columns is a wash, the intraday leg loses, customer_sue carries nothing

| Run | Return / Sharpe / max DD | Names / gross / turnover | IC | vs `_f0` paired (t, NW) | Blocks ≥ 0 | Gate |
|---|---|---|---|---|---|---|
| `_f0` live roster + customer_sue shadow | +1355% / 2.27 / 31.2% | 14.5 / 1.14 / 0.41 | 0.044 | — | — | base |
| `_f1` prior-only (diagnostic) | +111% / 0.82 / 35.2% | 11.1 / 0.91 / 0.53 | — | −44.5 bp/d (−3.29, −3.52) | 0/6 | diagnostic |
| `_f2` λ = 1000 | +982% / 2.17 / 30.4% | 12.9 / 1.05 / 0.41 | 0.055 | −7.4 bp/d (−0.94, −1.35) | 2/6 [−19.0, −22.9, −16.6, −3.3, +0.3, +14.2] | FAIL (both readings) |
| `_f3` direction-only columns dropped | +1359% / 2.34 / 30.9% | 13.6 / 1.08 / 0.41 | 0.052 | −0.4 bp/d (−0.08, −0.10) | 2/6 [−1.4, −10.6, −13.3, −4.2, +7.2, +9.7] | FAIL (consistency) |
| `_f4` intraday customer momentum | +1064% / 2.15 / 32.1% | 14.1 / 1.11 / 0.40 | 0.032 | −5.3 bp/d (−0.98, −0.93) | 2/6 [−13.0, −6.7, +17.9, −12.0, −3.7, +4.0] | FAIL |

Window: 470 common sessions 2024-10-02..2026-08-18 (the run day moved the window one session on; round 45's table was cut to 450 sessions by the 40-day-horizon run, so `_e0` and `_f0` are not comparable — every gate is same-day for this reason).

**Reading.** (1) `_f1`: the equal-weight prior blend, traded as it is, makes +111% where the fitted weights make +1355% (paired −44 bp/d, t −3.3): the learner's weights carry most of the book's return on this window, so the pre-registered "conditional verdicts" reading does not apply — and it is the sharpest statement yet of how much of the headline is the fitted combination on the window where λ, the horizons and the sizing were chosen. Part of the gap is sizing, not ranking: the equal blend's conviction scale differs from the fitted one, so it trades a smaller, more turbulent book (11 names, gross 0.91, turnover 0.53). (2) `_f2`: the CV's λ = 1000 raises the learned IC (0.044 → 0.055) and lowers the return by 7 bp/d with a smaller book (12.9 names, gross 1.05): stronger shrinkage shrinks the conviction scale and the sizing turns that into less exposure; the 2026-09-10 decision to overrule the CV stands on the corrected replay. A scale-preserving shrinkage (λ = 1000 with the convictions rescaled to the λ = 150 dispersion) would separate ranking from sizing — noted, not pre-registered. (3) `_f3`: dropping the direction-only columns of the 8 simulated agents (16 → 8 columns per horizon in the replay; 24 → 12 live) is a wash on return (−0.4 bp/d, t −0.1) with a higher Sharpe (2.34 vs 2.27), a lower drawdown (30.9% vs 31.2%) and a higher IC (0.052), but 2 of 6 blocks: FAIL under v4.1 and under the weaker reading (5/6 needed). The live version would also drop the three Claude agents' direction-only columns, which the replay cannot contain. Not adopted; recorded as the round's open question for the user (the simpler model at equal return). (4) `_f4`: the intraday leg (Wang 2025) transfers nothing: −5.3 bp/d, IC 0.044 → 0.032, and the worst rebound block does not improve (block 2: −6.7 bp/d against the pre-registered ≥ +7.6); the close-to-close measure stays. Closed. (5) `customer_sue` shadow: 31,384 rows, 30,482 scored, 61 names a day; event-study spread −0.11% per 20 d (overlap-adjusted t −0.15), 48% of dates positive, pooled rank IC +0.011; by half-year −1.33, −0.09, +0.03, −0.04, +2.31 (2026H2, 34 dates). Fails the promotion bar (≥ +0.4%, t ≥ 1, 4 of 5 half-years) on every count: the fundamentals channel of the customer links carries nothing here while the price channel does; the module stays in `agents/` unused and never becomes a voting candidate. Nothing adopted; no 2019–23 confirmation needed; VM stopped. Trials: 5 (4 replays + the shadow evaluation); `RESEARCH_TRIALS` → 355.

**Next (round 47 candidates, in the order the diagnostics suggest; one adoption a day):** (i) `events` without the pre-earnings leg; (ii) its drift leg restricted to the below-median-cap names; (iii) customer_momentum with a constant confidence; (iv) the conviction-EMA decision from the fills diagnostic after the 09-18 session; (v) scale-preserving shrinkage.

### Live book diagnostic (2026-09-18 01:10 PT, read-only; user: "레버 끼고도 SOXX보다 못 나가는데 문제 아니냐 … 추세선 아래라 저베타를 뽑은 거구나, 이제는 고베타를 뽑을 수 있겠네?")

Alpaca, 09-08..09-17 closes: the book +2.32% against SOXX −1.70%, QQQ −0.20%, SPY −0.44% — ahead of all three because it held ~0 beta through the −5.6% (09-14) and −2.7% (09-10) sessions, and it captures about a third of the up sessions (09-17: +1.25% against SOXX +3.39%; 09-11: +0.84% against +1.86%). Realised beta 0.12 over 7 sessions; position-weighted 60-day beta 0.18. Exposure at the 09-17 close: 15 names, gross 1.42, cash −42% (margin), SOXX sleeve 0 (SOXX is above its 200-day, +17.9%, but there is no idle equity); utilities 60% of equity (ETR 11.6, XEL 11.0, D 11.0, ES 10.9, EXC 9.1, AEP 8.9), plus SHEL, AMZN, MSFT, UBER, APD, MMM, CSCO; semiconductors only STM and TXN.

**Why the book is low-beta — corrected reading.** The regime was already risk-on on 09-17 (`macro`: +0.30 = SOXX below 50d −0.4, SPY above 200d +0.3, VIX 15 +0.3, NFCI loose +0.1) and macro therefore voted *against* the utilities (ETR −0.32, XEL −0.36, NEE −0.36) and for the high-beta names (STM +0.08). The live learner's weight on macro is **−0.067** and on supply_chain **−0.234**: the ridge trades both agents backwards, so macro's "sell low beta" becomes a +0.05 contribution to every utility and supply_chain's uniformly bullish statements become a −0.13 push on every name (a level shift the demeaning removes). The negative macro weight is what the beta-abnormal target teaches: in a rally a high-beta name's return is mostly β × SOXX and is stripped from the label, while a low-beta name with its own trend (the AI-power utilities) scores as pure alpha — so "against macro" = "long low-beta alpha". Consequence for the user's question: the book will **not** turn high-beta when SOXX crosses its 50-day average (it closed at 519.10 against a 526.32 average, still below, so the regime is unchanged for the 09-18 cycle anyway); a stronger risk-on reading makes macro's votes larger in the same direction and, through the negative weight, pushes the book further into low beta, until the daily refits (90-day half-life on top of the `_c3` warm start) change the sign. The sizing then levers the low-vol basket to the 1.5 ceiling (15% book vol never trips the 50% brake). Not a bug: the design's consequence, measured in round 42 on 09-17 (beta 0.12, 15% vol at 142% gross). The levers that exist: the trend-gated beta floor with the SOXX ETF (round 42 `_x2`: +1112% / 2.19 / 31.6% against +1090% / 2.18 / 30.2%, paired +0.4 bp/d, 3/6 blocks, 2019–23 −0.2 bp/d — neutral on average, more up-capture, +1.4 pts drawdown; FAIL under v4 on consistency, a user-decision candidate), the gross ceiling 2.0 (`_x1`: more return, lower Sharpe, FAIL), and a semiconductor-only universe (untested; round 47 candidate). Nothing changed; recorded for the user's decision.

### Fills diagnostic (2026-09-18 05:30 PT, `slippage.py` on the paper account's fills 09-10..09-17 against Alpaca's official opens; read-only)

| Window | Fills / notional | Adverse slippage per dollar traded | Buys / sells | Market / limit fills |
|---|---|---|---|---|
| pre-registered: fills ≤ 09:45 ET | 48 / $2.51M | **+23.4 bps** | +16.1 / +30.0 | +28.4 (41 fills) / −9.8 (7) |
| both execution slices: fills ≤ 09:50 ET (the second slice, sent at 09:40, fills at ~09:47) | 56 / $2.90M | **+17.2 bps** | +6.4 / +30.0 | +28.4 (41) / −16.8 (15) |
| later fills (the 09-16 afternoon run, the sleeve, clean-ups) | 31 / $3.20M | −30.3 (favourable, not open executions) | | |

By day (≤ 09:50): 09-10 +27.4, 09-11 +32.8, 09-14 +22.6, 09-17 +2.0 bps. Paper fills carry the spread but no impact: a floor for a real account.

**Reading and decision.** The pre-registered window (≤ 09:45) says +23.4 bps → adopt the conviction EMA; the window that actually covers the system's own open execution (two slices ten minutes apart) says +17.2 bps → below the 18.5 line. The 09:45 boundary was written without checking the second slice's fill time, so the rule was mis-specified and cannot decide on either number — recorded as a researcher error, not resolved by choosing the number one prefers. The EMA item stays **undecided**: re-measured after 30 live sessions (about 2026-10-22) with the window fixed now at ≤ 09:50 ET, stocks and sleeve included, the same 18.5 bps line; if the cost is above it then, the EMA (live implementation: today's convictions smoothed with the previous cycle's recorded convictions, a = 0.5) becomes the next adoption round; below, the item closes. Meanwhile the realised cost is 3–5 times the replay's 5 bps: at 0.41 turnover a day that is 7–10 bp/d, the largest single drag measured on the live book, so the reports' 15-bps column is the realistic one (+1,163% not +1,469% for the round-45 book). **Execution finding, live-only:** market fills cost +28 bps while limit fills earned −17 bps, and sells (+30) cost far more than buys (+6); 41 of 56 open fills went as market orders, meaning the marketable-limit path (needs a fresh IEX quote with a spread under 300 bps) rarely had a quote at 09:30. Candidate, measured by this diagnostic only: wait for the first quote (up to two minutes) before sending, and send exits as marketable limits with the same clean-up — an execution change, pre-registered here, decided on the next 30 sessions' fills, no replay.

### Entry-time study re-run (2026-09-18 07:20 PT; user: "매수매도 시점이 문제냐, 시가에 팔고 종가에 사거나 점심에 사는 건?"; `tmp/timing_now.py` on the `_c3` ledger, 8 simulated agents, beta target, refit every 5 days, the study's own sizing, 5 bps, 730 days of hourly bars, 470 sessions 2024-10..2026-08)

| Execution | Return / Sharpe |
|---|---|
| close of the signal day (the sweep's assumption; not tradable) | +2,869% / 2.64 |
| **whole book at 09:30 on t+1 (live practice)** | **+1,753% / 2.19** |
| 10:30 / 11:30 / 12:30 | +1,270% / 2.07 · +1,176% / 1.96 · +944% / 1.83 |
| 13:30 / 14:30 / 15:30 / close | +1,338% / 1.98 · +1,231% / 1.97 · +1,276% / 1.98 · +1,260% / 2.02 |
| exits 09:30, entries at the close | +1,269% / 2.17 |
| exits 09:30, entries at 12:30 | +1,434% / 2.21 |
| exits 09:30, entries at 10:30 | +1,324% / 2.08 |

Average intraday path of the equal-weight universe (bps per session): 09:30→10:30 +0.4, →11:30 −1.2, →12:30 +2.1, →13:30 +1.2, →14:30 +2.5, →15:30 −2.7, →close +1.5, **close→next open +20.4**: the universe's return accrues overnight, the intraday legs are noise. The open stays the best tradable time on the current ledger, as in the first study: the signals are computed from day t's close and decay through the next session, so every later entry forfeits the selected names' morning move (the whole book at 12:30 loses 46% of the multiple; entries at the close, 28%). The split the user proposed (exits at the open, entries at 12:30) is Sharpe-neutral (2.21 vs 2.19) and −18% of the multiple: not adopted. Execution quality at 09:30 (the fills diagnostic: market fills +28 bps, exits +30) is the cost lever, not the hour. No change to the 09:30 / 09:40 slices.
...
**Runs (VM, same day and machine, fixed replay, current main, snapshot 2026-09-22, price cache of 2026-09-25, the live configuration `--exec close --conviction-ema 0.5 --beta-floor 0.5 --cost-bps 20`, SOXX sleeve on its 200-day trend, margin 7%, 175 names):** 2024–26 `_fa0` daily (baseline), `_fa1 --rebalance-every 2`, `_fa2 --rebalance-every 3`, `_fa3 --rebalance-band 0.5` (new flag: a held name is resized only when its target moved by more than 50% instead of 30%); 2019–23 (`--days 1200 --end 2023-12-29 --agents technical,mean_reversion,risk,macro,events,customer_momentum`) `_fb0`..`_fb3` the same four, scored only for candidates that pass the 2024–26 gate. Weekly trading is not re-run (round 49: −11 bp/d and +10 points of drawdown).
...
**Why.** Diagnostics of 2026-09-25 (0 trials): the live book has run at beta 0.14–0.21 to SOXX, and rebuilding each live day's book from the recorded signals with only the 8 agents the replay simulates gives the same names (88–100% overlap) and the same beta (0.15–0.26), so the tilt comes from the validated model in this regime, not from the three Claude agents and fundamentals. Over 09-17→22 the book caught about 18% of SOXX's +14% (its beta); missed and failed cycles account for roughly 2 of the ~9 points it trails since 09-10 (the review's estimate). Sortino on log returns at 20 bps: 2024–26 EMA book 3.59 against SOXX 1.48, but 2019–23 EMA book 0.25 against SOXX 0.84 — outside the graph era the stock picks have not beaten holding the index. Round 47 chose floor 0.5 by the user's up-capture rule and failed 0.8 on drawdown by 0.9 points; the configuration has changed since (conviction EMA, which cut 2019–23 drawdown by 10 points; 20 bps; close execution; 175 names without SHEL), so the same rule is re-applied once, now.

**Runs (VM, same day and machine, fixed replay, current main, snapshot 2026-09-22, price cache of 2026-09-25, the live configuration: `--exec close --conviction-ema 0.5 --cost-bps 20`, SOXX sleeve on its 200-day trend, margin 7%):** 2024–26 `_y0 --beta-floor 0.5` (baseline = live), `_y1 --beta-floor 0.8`, `_y2 --beta-floor 1.0`; 2019–23 (`--days 1200 --end 2023-12-29 --agents technical,mean_reversion,risk,macro,events,customer_momentum`) `_z0` / `_z1` / `_z2` with the same three floors. No other level will be tried.

**Decision rule (round 47's, the user's up-capture criterion, unchanged):** adopt the **highest** floor whose 2024–26 Sharpe is not below `_y0`'s by more than 0.05 and whose max drawdown is within +3 points of `_y0`'s, and whose 2019–23 max drawdown is within +3 points of `_z0`'s; otherwise the floor stays 0.5. v4.1 figures (paired, blocks) are reported for information. Caveat fixed now: the replay's book usually carries beta near 1.1, so the floors bind on few replay days and the replay understates their live effect (the live book binds every uptrend day); the realised beta and up-capture are reported per run.

**Warm start (0 trials, a correctness fix, decided now):** `state/backtest.sqlite` is the `_c3` ledger of 09-17 (open execution, labels from the signal close, the old customer-momentum confidence, 151 names), while live now runs close execution with same-close labels, `MOMENTUM_CONF` 0.5, 175 names and the EMA. `_y0`'s ledger is the live configuration's own replay. It replaces the warm start if a rehearsal on a state copy shows the learner refitting without error and the book's names and gross within reason; the old file is kept as `backtest_c3_before_y0_<date>.sqlite`. Expected: floors 0.8 / 1.0 near-flat in Sharpe with +2–6 points of drawdown, so 0.8 may now qualify and 1.0 probably not. Trials: 2 (+ 2 confirmations); `RESEARCH_TRIALS` → 388.

### Round 53 results (2026-09-25 01:10 PT, VM, code 163532d, 20 bps, EMA 0.5, close execution, 175 names): the floor stays 0.5; the warm start is the live configuration's own replay from the 09-25 cycle

| Window | Floor | Return / Sharpe / Sortino / max DD | vs 0.5 paired (t) | Blocks ≥ 0 | Rule |
|---|---|---|---|---|---|
| 2024–26 (470 d) | `_y0` 0.5 | +1194% / 2.34 / 3.77 / 27.7% | — | — | base |
| 2024–26 | `_y1` 0.8 | +1135% / 2.29 / 3.65 / 30.2% | −0.9 bp/d (−0.45) | 1/6 | Sharpe −0.05, DD +2.5: within |
| 2024–26 | `_y2` 1.0 | +905% / 2.08 / 3.26 / 32.3% | −5.2 bp/d (−1.46) | 1/6 | Sharpe −0.26: out |
| 2019–23 (1,170 d) | `_z0` 0.5 | +30% / 0.17 / 0.24 / 52.7% | — | — | base |
| 2019–23 | `_z1` 0.8 | +17% / 0.10 / 0.14 / 55.9% | −0.8 bp/d (−0.91) | 0/5 | **DD +3.2 > +3: out** |
| 2019–23 | `_z2` 1.0 | +21% / 0.12 / 0.16 / 57.2% | −0.3 bp/d (−0.23) | 1/6 | out |

**Decision (the rule as written).** No higher floor qualifies: 0.8 misses the 2019–23 drawdown limit by 0.2 points, 1.0 loses Sharpe and Sortino in both windows. The floor stays 0.5. More beta does not buy risk-adjusted return in either window; the live book's low beta is the model's current regime choice (the replay's book averaged beta ~1.1 in 2024–26), and the fix for lagging a semiconductor rally is not a higher floor on this evidence. **Warm start:** rehearsed on state copies (old `_c3` against `_y0`): the learner refits without error, the weights keep their structure (supply_chain −0.239 → −0.208, neighbors +0.191 → +0.183, customer_momentum +0.140 → +0.123, mean_reversion +0.054 → +0.104), the beta floor binds the same way (SOXX 42% → 38%), 2 orders either way. Swapped in the live state at 01:30 PT (`backtest_c3_before_y0_20260925.sqlite` kept). A rehearsal caught that a diagnostic's lookback-200 `market.closes` call had written the day's closes cache first (139 rows: the floor off, "no usable SOXX closes"); the file was removed and `market.closes` now refetches a cache that does not cover the requested window (5244c11). Trials: 2; `RESEARCH_TRIALS` 388.

## 2026-09-22 20:25 PT — round 52, pre-registered before any result (user: "우리 지금 거래비용 너무 많이 내냐?", then "일단 니가 알아서 찾아내줘"): trading less at the measured cost of 20 bps

**Why.** Measured today with `slippage.py`: the 72 open-window fills of 09-10..09-18 cost +19.3 bps per dollar traded against the official open ($4.05M); the 24 fills of 09-22 cost +23.8 bps against the official close ($1.59M; the after-hours limits +26.3, the in-session market orders +5.2). The replay charges 5 bps per dollar traded one-way, the same unit. Live turnover averaged 114% of equity a day over the 8 sessions (Alpaca fills; inflated by one-offs such as the SOXX sleeve's entry and exit and the regime switches; the last three cycles 130–154%) against the replay's 0.41–0.46. Round 49 kept daily rebalancing because close execution was expected to remove the spread through market-on-close orders filled at the official close; on 09-22 the paper account filled 1 of 19 MOC orders, and the close refresh now crosses the spread with marketable orders (`CLOSE_ORDER_TYPE = "market"`, 4e7300a). That voids round 49's premise and both measurements pre-registered for about 10-22 (the open window no longer trades; the MOC slippage no longer exists). This round therefore decides at the measured cost.

**Runs (VM, same day and machine, fixed replay, current main, snapshot 2026-09-22, price cache of 2026-09-22, the live configuration: `--exec close --beta-floor 0.5`, SOXX sleeve on its 200-day trend, margin 7%, 176 names, `MOMENTUM_CONF` 0.5 from config), all at `--cost-bps 20`:** `_t0` baseline (daily rebalancing); `_t1 --rebalance-every 2` (round 49's best at 20 bps: +1.4 bp/d, 5/6 blocks); `_t2 --conviction-ema 0.5` (round 45's `_e5`: turnover 0.41 → 0.24). The 2019–23 confirmation for each pass: `--days 1200 --end 2023-12-29 --agents technical,mean_reversion,risk,macro,events,customer_momentum`, same flags, 20 bps, against its own baseline `_u0`. Seat protection and exit hysteresis are not re-run: rounds 8 and 12–15 found them worse or inside noise, and they act on the same cutoff churn as the EMA.

**Decision rule.** Gate v4.1 at 20 bps against `_t0` (full-window paired ≥ 0, Sharpe not lower, max DD ≤ base + 2 pts, at least two thirds of the informative blocks not worse), then the 2019–23 confirmation (paired ≥ 0, max DD ≤ base + 3 pts). If both candidates pass, adopt the one with the larger full-window paired difference; within 0.5 bp/d of each other, the EMA, because the replay has no Claude agents, whose day-to-day re-draws (llm_news: day-over-day correlation +0.45) add live churn at the top-15 cutoff that the EMA damps and a fixed schedule does not. If neither passes, daily rebalancing stays and the cost is attacked on the execution side (limit orders at the mid in the close window with the market clean-up; live-only, measured, no replay). Revert check, fixed now: after 30 sessions of close-window fills, if their measured cost is below 12 bps, the adopted candidate is re-run at the measured cost and reverts if it fails. The reports' 5 / 15 / 30 bps columns are information. Expected: `_t1` about +1 bp/d and `_t2` about 0 at 20 bps, both inside noise. Trials: 2 (+ up to 2 confirmations); `RESEARCH_TRIALS` → 386.

### Round 52 results (read 2026-09-23 16:25 PT, VM, code 2515d56, gate v4.1, 20 bps; the batch finished overnight, the session was suspended until the afternoon): the conviction EMA passes both windows and is adopted; rebalancing every 2 sessions fails

| Window | Run | Return / Sharpe / max DD | Turnover | vs baseline paired (t, NW) | Blocks ≥ 0 | Gate |
|---|---|---|---|---|---|---|
| 2024–26 (470 d) | `_t0` daily | +770% / 1.94 / 28.6% | 0.50 | — | — | base |
| 2024–26 | `_t1` every 2 sessions | +778% / 1.93 / 27.6% | 0.35 | +0.3 bp/d (+0.07, +0.07) | 4/6 | FAIL (Sharpe −0.01) |
| 2024–26 | `_t2` conviction EMA 0.5 | **+1016% / 2.24 / 28.7%** | 0.32 | **+4.9 bp/d (+1.26, +1.10)** | **6/6** [+6.1, +0.9, +2.5, +9.2, +15.2, +0.2] | **PASS** |
| 2019–23 (1,170 d) | `_u0` daily | −24% / −0.17 / 60.5% | 0.50 | — | — | base |
| 2019–23 | `_u2` conviction EMA 0.5 | **+31% / 0.18 / 50.5%** | 0.28 | **+4.5 bp/d (+2.80, +3.15)** | 5/6 | **PASS** (DD −10.0 pts) |

Cost columns (the same trades re-costed; information): 2024–26 `_t0` 0 / 5 / 15 / 30 bps = +1295% / +1140% / +879% / +586% (Sharpe 2.37 / 2.26 / 2.05 / 1.73); `_t2` +1399% / +1292% / +1101% / +862% (2.51 / 2.44 / 2.31 / 2.10); `_t1` +1119% / +1023% / +854% / +645%. 2019–23 `_u0` +143% / +82% / +1% / −58%; `_u2` +153% / +115% / +55% / −5%. Deflated Sharpe (384 trials) 0.847 for `_t2` against 0.728 for `_t0`. (`_u1`, the every-2 confirmation, ran in the same batch to save time and is not scored: `_t1` failed the first gate.)

**Reading and decision (the pre-registered rule).** Only the EMA passes, in both windows, and it is adopted: `CONVICTION_EMA = 0.5` from the 09-24 cycle. It beats daily rebalancing at every cost level in both windows — at 0 bps too — so under close execution the smoothing improves the traded ranking itself (less day-to-day noise at the top-15 cutoff), not only the cost; round 45's −2.3 bp/d at 5 bps was measured under open execution with the 09-17 roster. Rebalancing every 2 sessions is cost-neutral at 20 bps and worse below it. Two sober points: at the measured cost the 2019–23 book without the graph agents loses money on daily rebalancing (−24%), and the EMA lifts it only to +31% with a 50% drawdown — the costs matter more than any signal tested since round 45; and the revert check stands (after 30 sessions of close-window fills, a cost below 12 bps re-runs this round at the measured cost; the EMA is ahead at 5 bps as well, so a revert is unlikely). Live implementation: `cycle.smooth_convictions` (before demeaning, as the replay; previous cycle's smoothed values in `state/conviction_ema.json`; a same-day re-run smooths against the same previous day; a previous day more than 7 days back is not used), seeded with 09-22's pre-demean convictions (the 12-agent book the late run traded) so the 09-24 cycle smooths against it. The `alt_conf` and `raw_target` paper twins compute their own convictions without the EMA and now differ from the live book by the EMA as well. Trials: 2; `RESEARCH_TRIALS` 386.

## 2026-09-18 18:10 PT — paper twins (virtual counterfactual books) and the collapsed adoption schedule (user decision)

**Why.** The user asked whether to run several paper accounts with several strategies. Extra accounts would not add information for the money: the Claude stage is the cost (~526 calls a day) and every sizing / execution / roster variant can share it; a live A/B has no power to separate variants that differ by 1–5 bp/d against a book that moves 2–3% a day; and Alpaca gives one paper account per login. What a live track *can* do is catch plumbing and execution failures and accumulate the record for the Claude agents, and that only needs the counterfactual books, not accounts.

**Design (`paper_twins.py`).** Each cycle builds five books from the same signals and records their target weights: `live_model` (the live targets, stocks and sleeve, marked like the replay — the strategy's own curve beside the account's, so the gap is execution and tracking), `open_exec` (the same targets traded at the next open and held to the following open — the pre-09-21 execution), `no_floor` (the live sizing without the beta floor), `alt_conf` (customer_momentum under the other confidence rule), `raw_target` (the raw-return shadow model under the live sizing). Books are marked two sessions later at official prices (closes from the cycle's frame; opens from Alpaca's daily bars) with drifted holdings and `COST_BPS` per dollar traded — the replay's conventions, so a twin's curve is comparable to a replay's. The dashboard payload carries `twins`; the cycle notes print the running returns. The record is meant to be read after months, not days; the pre-registered use is (a) the tracking error of the account against `live_model` (execution quality), (b) the sign of `live_model − open_exec` as the live evidence for round 48, (c) `live_model − no_floor` as the live cost of the floor, (d) `live_model − alt_conf` for round 47's confidence change. No twin is ever promoted without a replay round.

**Schedule (user: "월-수까지 기다릴 필요가 없잖아").** With the controls recorded daily, the one-change-a-day staggering loses its purpose: the three adopted changes — the beta floor 0.5, close execution, the constant customer-momentum confidence — go live together in the 09-21 cycle (the switch on Sunday 09-20 evening: `EXEC_MODE = "close"`, `LABEL_SAME_CLOSE_FROM = "2026-09-21"`, `MOMENTUM_CONF = 0.5`, the task at 11:30 PT), each with its twin as the counterfactual. Recorded as the user's decision; the measured costs stand as recorded in rounds 47–48.

## 2026-09-18 17:00 PT — round 51, pre-registered before any result (user: "테스팅하거나 향상시킬 거 있냐"): a five-day horizon under close execution, and λ = 1000 at equal conviction dispersion

**Why.** (1) The five-day label was dropped in v2.3 as noise under open execution, where the signal is 17 hours old at the fill; under close execution (live from 09-22) the signal is minutes old, so the shortest horizon is the untested side again. (2) Round 46's `_f2` (λ = 1000, the walk-forward CV's own choice) raised the learned IC from 0.044 to 0.055 and lost 7 bp/d because the stronger shrinkage shrank the conviction scale and the sizing turned that into a smaller book. Rescaling the day's demeaned convictions to a fixed cross-sectional dispersion (`--conviction-zscore K`) holds the sizing constant, so a λ comparison at equal dispersion isolates the ranking. K = 0.08 for both arms (one value, chosen before any run as roughly the live convictions' dispersion; not tuned).

**Runs (VM, same day and machine, fixed replay, `--exec close --beta-floor 0.5`, margin 7%, universe 176 and snapshot 2026-09-18 from this round on — the VM's replay inputs synced today):** `_s0` baseline; `_s1 --horizons 5,10,20`; `_s2 --conviction-zscore 0.08` (the rescaling alone, λ 150); `_s3 --prior-strength 1000 --conviction-zscore 0.08`. Gates: `_s1` vs `_s0` (v4.1); `_s2` vs `_s0` (v4.1: is the rescaling itself neutral?); `_s3` vs `_s2` (v4.1: the ranking question at equal dispersion); 2019–23 confirmation for passes. Expected: `_s1` inside ±2 bp/d; `_s2` near neutral; `_s3` vs `_s2` positive if the IC gain is real (+1–3 bp/d), else λ = 1000 is closed for good. Adoption: one change a day after the queued three (floor 09-21, close execution 09-22, constant confidence 09-23). Trials: 3 (+ confirmations); `RESEARCH_TRIALS` → 384 at the close.

### Round 51 results (2026-09-18 17:45 PT, VM, code bcd5b70, gate v4.1; the first batch on the 176-name universe and the 2026-09-18 snapshot): nothing passes; the five-day horizon and λ = 1000 are closed; forcing a constant conviction dispersion is destructive

| Run | Return / Sharpe / max DD | Gross / turnover | IC | vs base paired (t, NW) | Blocks ≥ 0 | Gate |
|---|---|---|---|---|---|---|
| `_s0` baseline (close execution, floor 0.5, 176 names) | +1338% / 2.41 / 26.2% | 1.20 / 0.52 | 0.045 | — | — | base |
| `_s1` horizons 5 / 10 / 20 | +1226% / 2.34 / 27.1% | 1.19 / 0.55 | 0.045 | −1.7 bp/d (−0.69, −0.62) vs `_s0` | 2/6 | FAIL |
| `_s2` convictions rescaled to std 0.08 (λ 150) | +659% / 1.88 / 43.1% | 1.15 / 0.49 | 0.045 | −14.1 bp/d (−2.72, −2.37) vs `_s0` | 1/6 [−3.1, −57.4, +0.4, −6.4, −10.3, −6.9] | FAIL |
| `_s3` λ 1000, rescaled to std 0.08 | +619% / 1.87 / 45.1% | 1.14 / 0.57 | 0.052 | −1.4 bp/d (−0.31, −0.38) vs `_s2` | 2/6 | FAIL |

**Reading.** (1) The universe expansion moved the same-configuration baseline from +1709% / 2.42 / 27.3% (151 names, snapshot 09-17) to +1338% / 2.41 / 26.2% (176 names, snapshot 09-18): the same Sharpe, a lower drawdown, a smaller multiple — the 25 new names dilute the winners of 2025–26 and the new snapshot's statements change the graph agents' inputs. Every later round compares within its own batch, so nothing recorded changes; the website's report should be refreshed from a 176-name replay once close execution is live. (2) A five-day horizon adds nothing even with minutes-old signals: closed. (3) Rescaling the day's demeaned convictions to a fixed dispersion is destructive (−14 bp/d, the crash block −57 bp/d, drawdown 43%): the cross-sectional size of the convictions is a signal about the day — a weak-signal day produces a small book under the live sizing, and the rescaling fills it. The sizing's "conviction × 0.60" is therefore doing real work, not just scaling. (4) At equal dispersion λ = 1000 still returns nothing for its higher IC (0.052 vs 0.045): the CV-preferred shrinkage improves the ranking's correlation but not the money, on the corrected replay as on the old one; closed for good, with the 2026-09-10 override confirmed twice. Nothing adopted. Trials: 3; `RESEARCH_TRIALS` 384.

## 2026-09-18 15:40 PT — round 50, pre-registered before any result (user: "섹터 포트 에이전트도 필요할까? 대부분 반도체라 상관없나?"): chain momentum as a vote, not a sector allocation

**Why.** The universe is the whole AI supply chain (176 names: semiconductors, power and utilities, hyperscalers, EMS, materials), and the book's sector split is set bottom-up by the learner. Fixed allocations were measured and lost (round 39 chain-neutral demeaning −11 bp/d in its test window; round 47 chain cap a wash, semiconductor-only −5.3 bp/d). What has not been tested is the sector signal itself: industry momentum (Moskowitz & Grinblatt 1999: the six-month return of an industry predicts its next month; strong in the original sample, decayed since). `agents/chain_momentum.py`: for each name, the equal-weight trailing 126-session return of the universe names sharing its graph chain tags (hbm, foundry, neocloud, power_cooling, …), averaged over its tags, minus the universe's mean; direction = tanh(4 × that), confidence 0.5, horizon 20; silent without a chain of ≥ 3 priced members. Prices only (point-in-time); the tags are today's graph. Expected to share customer_momentum's momentum-crash signature.

**Runs (VM, same day and machine, fixed replay, `--exec close --beta-floor 0.5`, margin 7%, snapshot 2026-09-17):** `_r0` baseline with `--shadow chain_momentum` (recorded, scored, non-voting: event-study spread and IC with `insider_eval.py`), `_r1 --extra chain_momentum` (voting). Gate v4.1 for `_r1` against `_r0`; 2019–23 confirmation on a pass. Shadow bar as for customer_sue: spread ≥ +0.4% per 20 days with t ≥ 1 and positive in ≥ 4 of 5 half-years for the agent to stay a candidate at all. **Decision rule:** the vote is adopted only on a v4.1 pass plus the confirmation; a shadow that clears its bar without a voting pass runs live as a shadow; otherwise the agent stays in `agents/` unused. Expected: a positive spread of about half the published industry-momentum effect (~+0.3% per 20 days), a voting result that fails block consistency in the rebound blocks. Trials: 2 (+1 shadow evaluation); `RESEARCH_TRIALS` → 381 at the close.

### Round 50 results (2026-09-18 16:20 PT, VM, code 00f548a, gate v4.1, close execution with the beta floor): the sector signal carries information, the sector vote hurts the book

| Run | Return / Sharpe / max DD | IC | vs `_r0` paired (t, NW) | Blocks ≥ 0 | Gate |
|---|---|---|---|---|---|
| `_r0` baseline + chain_momentum shadow | +1709% / 2.42 / 27.3% | 0.039 | — | — | base |
| `_r1` + chain_momentum voting | +1431% / 2.23 / 31.8% | 0.037 | −3.3 bp/d (−0.87, −0.88) | 2/6 [−6.2, −8.7, +5.2, −6.5, −8.2, +4.8] | FAIL |

Shadow evaluation (`_r0` ledger): 75,500 rows, 73,246 scored at 20 days, 146.5 of 151 names a day — the agent votes on almost every name, so the pre-registered "signalled names against the rest" spread is undefined (0 usable dates) and cannot clear its bar; the ranking statistic that remains is the pooled rank IC of **+0.072** (customer_momentum +0.097, customer_sue +0.011, insider −0.08): the chain's six-month return does rank next month's names. **Reading.** A sector signal exists, but as a vote it is a between-chain tilt that stacks on the same rotation risk the book already carries (utilities against semiconductors, customer momentum): −3.3 bp/d, Sharpe 2.42 → 2.23, drawdown +4.5 points, four of six blocks worse. **Decision (the rule as written):** not adopted as a vote; the shadow bar was not met as defined, so the agent stays in `agents/` unused — the IC is on record for a later, pre-registered design (a smaller confidence or a within-chain use), not tuned now. The answer to the user's question: no sector-portfolio agent; the sector split stays bottom-up from the twelve voters, and the index exposure the user wants comes from the beta floor, not from a sector layer. Trials: 2 (+1); `RESEARCH_TRIALS` 381.

## 2026-09-18 14:25 PT — round 49, pre-registered before any result (user: "리밸런싱을 줄일까, 아니면 할 필요가 없나, 주 2회로 할까, 매수매도 시 호가 때문에 손해를 보나 — 니가 알아서 해줘"): rebalancing every 1 / 2 / 3 / 5 sessions under close execution, at 5 bps and at 20 bps

**Why.** The book turns over 0.41 of equity a day and the open fills cost 17–23 bps per dollar traded (market fills +28, limit fills −17: the spread and the opening drift are real). Close execution (round 48, live from 09-22) removes most of that — market-on-close orders fill at the official close without crossing a spread — so the frequency question is asked in that regime: daily (as now) against every 2, 3 (≈ twice a week) and 5 sessions (weekly), the book holding its drifted weights between trade days while the learner keeps fitting daily. Round 45's conviction EMA (a softer version of the same idea) was −2.3 bp/d at 5 bps and ahead at 30 bps; rounds 12–15 rejected sizing hysteresis on the old cost model.
...
**Runs (VM, same day and machine, fixed replay, margin 7%, snapshot 2026-09-17, live config incl. the beta floor 0.5):** `_j0 --exec open --open-refresh --beta-floor 0.5` (the 09-21 live configuration) against `_j1 --exec close --beta-floor 0.5` (no open refresh in close mode: the signals are the close's); 2019–23 `_k0` / `_k1` the same pair. Gate v4.1 on the paired daily difference (a close-to-close book against an open-to-open book, both daily), Sharpe, drawdown, blocks; the confirmation on `_k1` vs `_k0`. Staleness caveat: the replay's close mode uses the exact close for the price agents while the live design uses 15:45 prices — the 5-, 20- and 60-day measures move little in fifteen minutes; recorded, not simulated. Expected: the gap survives at half strength at least (+30–60% of the multiple); the live proof is the fills' slippage against the official close (target ≈ 0). Trials: 2 (+2 confirmations).

**Live design (built only on a pass; the user approved the midday PC and Claude use):** the scheduled task moves to 11:30 PT (14:30 ET): rule agents, the Claude stage (~50 min; its deadline becomes 15:35 ET instead of 09:05), then a 15:45 ET price refresh of the refresh agents (the open refresh, moved to the close), convictions, targets, the sleeve and the beta floor as now, and one slice of market-on-close orders at 15:46–15:48 (exits and entries fill together in the auction); labels scored from the signal day's close (aligned with `--exec close`); the guardian unchanged. Adoption is a schedule change: 09-22 at the earliest, after tests and a rehearsal on a state copy, one change a day (09-21 the beta floor, 09-22 close execution, 09-23 the constant confidence).

### Round 48 results (2026-09-18 09:10 PT, VM, code a523817, gate v4.1): close execution passes both windows

| Window | Open execution (base) | Close execution | Paired (t, NW) | Blocks ≥ 0 | β / up-capture / worst month | Gate |
|---|---|---|---|---|---|---|
| 2024–26, 470 sessions (`_j0` / `_j1`) | +1196% / 2.17 / 31.3% | **+1709% / 2.42 / 27.3%** | +7.4 bp/d (+0.43, +0.87) | 4/6 [−9.1, +3.5, +0.5, −6.0, +21.7, +7.0] | 1.13 / 1.86 / −16.8% → 1.12 / 2.10 / −13.7% | **PASS** |
| 2019–23, 1,170 sessions (`_k0` / `_k1`) | +65% / 0.34 / 47.1% | **+111% / 0.49 / 43.6%** | +2.3 bp/d (+0.44, +0.84) | 3/6 | 0.73 / 0.78 → 0.70 / 0.81 | confirmation **PASS** (paired ≥ 0, DD −3.5) |

Turnover 0.41 → 0.46 (the close book rebalances on fresher signals); learned IC 0.047 → 0.039 (the labels change with the convention). The gap is smaller than the entry-time study's (+2,869% with the study's own sizing) but the shape is the same: more return, higher Sharpe, lower drawdown, better up-capture, on both windows. Caveats carried into the live design: the replay's price agents use the exact close (live: 15:45 ET prices), and the closing auction is charged the replay's 5 bps (live target ≈ 0 against the official close; the open fills cost 17–23 bps). **Decision: build close execution for the 09-22 cycle** (schedule 11:30 PT, MOC orders, labels from the prediction day's own close, Claude deadline 15:35 ET), inert behind `config.EXEC_MODE = "open"` until adopted, with tests and a rehearsal on a state copy; the adoption order stands (09-21 floor, 09-22 close execution, 09-23 constant confidence). Trials: 2 + 2 → `RESEARCH_TRIALS` 370.

## 2026-09-18 01:25 PT — round 47, pre-registered before any result (user: "베타 좀 올려볼까 그러면은, 아니면 반도체랑 전력기기 비중을 세팅을 해놓던가"): beta floor at two levels, a power-chain cap, a semiconductor-only universe, and the two round-46 fixes

**Why.** The live book is a levered low-beta basket (beta 0.12–0.18, utilities 60% of equity) because the ridge trades macro and supply_chain backwards; the user wants either more beta or an explicit semiconductor / power split. Both are tested against the same-day baseline with the 09-18 roster, so the user chooses with numbers. The session's prior: the beta floor is the cleaner lever (it leaves the learner's ranking alone and adds index exposure only while SOXX is above its 200-day average); a chain cap overrides the learner's best relative bets and is expected to cost what round 39's chain-neutral demeaning cost (−11 bp/d in its test window).

**Runs (VM, same day and machine, fixed replay, margin 7%, snapshot 2026-09-17, gate v4.1; 2019–23 confirmation for passes):**
- `_h0` baseline (live roster of 09-18).
- `_h1 --beta-floor 0.5` and `_h2 --beta-floor 0.8` (round 42's mechanism, `portfolio.beta_floor`: while SOXX is above its 200-day average the book's beta is raised to the floor with SOXX; past the gross ceiling the stocks shrink together). Two levels pre-registered because the user asked for more up-capture; no other value will be tried. Report per run the realised beta and the return in the months SOXX rose (the user's criterion) next to the gate.
- `_h3 --chain-cap 0.30` (new `portfolio.chain_cap`: the power group — names whose only chain tag is power_cooling — may hold at most 30% of equity together, scaled down proportionally; the freed capacity is idle and the sleeve rule may fill it).
- `_h4 --exclude-group power` (the power group removed from the universe: a semiconductor-only book; its equal-weight line changes with it, so only the paired test against `_h0` counts).
- `_h5 --no-events-pre-leg` (round 46 diagnostic (c): `events` casts no pre-earnings vote).
- `_h6 --momentum-conf 0.5` (round 46 diagnostic (b): customer_momentum's confidence constant, the count no longer a feature).
Pass numbers: v4.1 as written for each against `_h0`. For `_h1`/`_h2`/`_h3`/`_h4` the decision is the user's on the measured return / Sharpe / drawdown / up-capture even when the gate fails (a risk-preference choice, recorded as such, one adoption a day, with live wiring, tests and a rehearsal before it trades). Hypotheses: `_h1`/`_h2` raise up-capture and drawdown with a flat Sharpe (round 42: +0.4 bp/d, +1.4 pts DD at 0.5); `_h3`/`_h4` lose return (the utilities were the book's best relative bets in 2025–26); `_h5` inside ±2 bp/d; `_h6` inside noise. Trials: 7; `RESEARCH_TRIALS` → 362 at the close.

**Decision rule, written 01:30 PT before any result (the user delegated the choice: "걍 니가 알아서 골라 맞는 거, 나는 자러 갈 거임").** (1) `_h5` / `_h6`: adopted only on a v4.1 pass, else closed. (2) `_h3` / `_h4`: adopted only on a v4.1 pass — they override the learner's ranking, so a failed gate means the cost is real. (3) `_h1` / `_h2`: the user's goal is up-capture, so the gate is the user's criterion, fixed here: the **higher** floor is adopted whose Sharpe is not below the baseline's by more than 0.05, whose max drawdown is within +3 points, and whose 2019–23 confirmation (run after the gate) keeps max drawdown within +3 points of that window's baseline — the 2022 bear is the test of the trend gate; if neither level qualifies, no beta change, finding recorded. (4) If a floor qualifies and (1)/(2) also pass, the floor goes first (the stated goal); the rest queue one a day. (5) Anything adopted needs live wiring, tests and a rehearsal on a state copy before the 09-21 cycle (Monday; 09-19 is a Saturday); if that is not done and reviewed by 22:00 PT, the adoption waits a day.

### Round 47 results, 2024–26 (2026-09-18 01:45 PT, VM, fixed replay, code 7b866c3, gate v4.1): one pass — the constant customer-momentum confidence; the beta floor 0.5 qualifies on the user's criterion pending 2019–23; the rest closed

| Run | Return / Sharpe / max DD | Gross / turnover | vs `_h0` paired (t, NW) | Blocks ≥ 0 | Gate v4.1 | Realised β / up-capture / worst month |
|---|---|---|---|---|---|---|
| `_h0` baseline | +1221% / 2.19 / 29.6% | 1.13 / 0.41 | — | — | base | 1.13 / 1.85 / −14.5% |
| `_h1` beta floor 0.5 | +1196% / 2.17 / 31.3% | 1.14 / 0.41 | −0.4 bp/d (−0.68, −0.69) | 2/5 informative, 1 tie [0, −0.2, +1.1, +0.5, −0.0, −4.8] | FAIL | 1.13 / 1.86 / −16.8% |
| `_h2` beta floor 0.8 | +1226% / 2.18 / 33.5% | 1.17 / 0.43 | +0.2 bp/d (+0.11, +0.12) | 2/5, 1 tie [0, −3.4, +6.8, −0.2, +1.4, −9.1] | FAIL | 1.17 / 1.89 / −19.5% |
| `_h3` power-chain cap 30% | +1205% / 2.22 / 29.9% | 1.05 / 0.39 | −0.5 bp/d (−0.22, −0.20) | 4/6 [+4.0, −1.4, −9.1, +7.6, +0.2, +0.1] | FAIL (paired < 0) | 1.12 / 1.87 / −14.9% |
| `_h4` semiconductor-only universe | +930% / 1.98 / 37.1% | 1.08 / 0.38 | −5.3 bp/d (−0.87, −0.76) | 2/6 | FAIL | 1.16 / 1.89 / −25.1% |
| `_h5` events without the pre-earnings leg | +1206% / 2.18 / 30.1% | 1.13 / 0.41 | −0.2 bp/d (−0.48, −0.48) | 3/6 | FAIL | 1.12 / 1.84 / −14.9% |
| `_h6` customer_momentum confidence 0.5 | +1355% / 2.28 / 30.4% | 1.13 / 0.41 | **+2.0 bp/d (+1.28, +1.33)** | 4/6 [−0.1, +0.5, −0.6, +7.0, +2.4, +0.9] | **PASS** | 1.12 / 1.93 / −15.6% |

(up-capture = the book's mean monthly return in the 13 months SOXX rose over SOXX's; in SOXX-down months the baseline made +37% against SOXX −37%.)

**Reading.** (1) The replay's book already carries beta 1.13 to SOXX, so the floors bind only in a few stretches and change little (block 1 is a tie); the replay cannot price the floor for the *live* book (beta 0.12–0.18, utilities 60%), whose 12-agent learner with the three Claude agents, fundamentals and the negative macro weight the replay does not contain. Live the floor binds every day SOXX is above its 200-day average: with stock gross 1.42 and beta 0.18 it scales the stocks by 0.81 and holds 35% of equity in SOXX (gross 1.5, beta 0.5). On the rule's numbers `_h1` qualifies (Sharpe −0.02, drawdown +1.7 pts), `_h2` does not (+3.9); the 2019–23 confirmation `_i1` vs `_i0` decides. (2) The chain cap is a wash with a smaller book (gross 1.05, Sharpe 2.22, 4/6) but paired −0.5 bp/d: rule (2) needs a pass, not adopted. (3) The semiconductor-only universe loses 5.3 bp/d with a 37% drawdown: the utilities are the book's diversifier; closed. (4) Removing events' pre-earnings vote changes nothing (the learner's negative weight on events was already trading it backwards); closed. (5) The constant confidence for customer_momentum passes v4.1 as diagnostic (b) predicted (the count diluted the direction); confirmation `_i3` on 2019–23; adoption queued behind the floor by rule (4). Trials: 7 (+ 4 confirmations).

### Round 47, 2019–23 confirmations and the decision (2026-09-18 02:50 PT, VM; 1,170 sessions 2019-04-09..2023-11-29, five price agents + customer_momentum)

| Run | Return / Sharpe / max DD | vs `_i0` paired (t, NW) | Blocks ≥ 0 | Realised β / up-capture / down-capture | Confirmation |
|---|---|---|---|---|---|
| `_i0` baseline | +75% / 0.38 / 46.9% | — | — | 0.71 / 0.76 / 0.87 | base |
| `_i1` beta floor 0.5 | +65% / 0.34 / 47.1% | −0.5 bp/d (−0.78, −0.79) | 1/5, 1 tie | 0.73 / 0.78 / 0.93 | max DD +0.2 ≤ +3: **qualifies** (rule 3) |
| `_i2` beta floor 0.8 | +70% / 0.36 / 47.8% | −0.1 bp/d (−0.08) | 2/5, 1 tie | 0.78 / 0.84 / 1.02 | out already (2024–26 DD +3.9) |
| `_i3` customer_momentum confidence 0.5 | +75% / 0.38 / 47.6% | +0.05 bp/d (+0.11, +0.11) | 3/6 | 0.71 / 0.77 / 0.89 | paired ≥ 0, DD +0.7 ≤ +3: **PASS** |

**Decision (the pre-registered rule, applied as written).** `BETA_FLOOR = 0.5` from the 09-21 cycle (2024–26: Sharpe −0.02, DD +1.7; 2019–23: DD +0.2, the trend gate switched the floor off through the 2022 bear; cost −0.4 / −0.5 bp/d on the two windows — the price of up-capture the user asked for). `MOMENTUM_CONF = 0.5` (v4.1 pass, confirmation pass) queued for the 09-22 cycle by rule (4). Live wiring: `portfolio.apply_beta_floor` + the cycle hook (tests in `test_round47_live.py`); rehearsed 02:05 PT in a dry cycle on a copy of the live state with the Claude calls blocked: the floor read the dry book's beta (0.47 with the rule agents only), planned a SOXX sleeve order and scaled the stocks by the ceiling, exit 0, nothing sent. On the live book (beta 0.18, stock gross 1.42) the floor will hold about 35% of equity in SOXX with the stocks scaled ×0.81 while SOXX stays above its 200-day average (it is, +18%). Not adopted: the floor 0.8, the chain cap, the semiconductor-only universe, the events pre-leg switch. Trials: 7 + 4 → `RESEARCH_TRIALS` 366.

**The user's structural question ("계속 유틸만 포지션 잡을 것 같은데, 고칠 방법 없나").** The raw-return shadow model (`abnormal` label, fitted daily beside the beta model since 2026-09-15, `shadow_targets`) wanted the same book on 09-17 — SHEL, D, AMZN, MSFT, ETR, STM, XEL, META, TXN, CSCO, UBER, LUMN, AEP, APD, ES, position-weighted beta 0.16 against the active model's 0.13 — so the learning target is not the cause and switching it would change nothing. The tilt is the agents' information plus the learned signs (neighbors +0.18 loves the utilities' hyperscaler customers, supply_chain −0.23 penalises the names with bullish statements, i.e. the semiconductors, macro −0.07). Non-negative weights were measured in the early rounds at +172% / 1.01 against the fitted learner's multiples (line 416): far worse. What remains is (a) index exposure — the floor adopted above — and (b) the learner's own rotation: the weights refit daily with a 90-day half-life on top of the `_c3` warm start, so a stretch in which semiconductors beat the utilities on a beta-adjusted basis moves the book within weeks, not days. This is a tilt the cross-sectional learner is meant to take, unlike the September cash problem (a level bias, fixed by demeaning).
