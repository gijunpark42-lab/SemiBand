...
**Runs (VM, same day and machine, fixed replay, margin 7%, snapshot 2026-09-17, live config incl. the beta floor 0.5):** `_j0 --exec open --open-refresh --beta-floor 0.5` (the 09-19 live configuration) against `_j1 --exec close --beta-floor 0.5` (no open refresh in close mode: the signals are the close's); 2019–23 `_k0` / `_k1` the same pair. Gate v4.1 on the paired daily difference (a close-to-close book against an open-to-open book, both daily), Sharpe, drawdown, blocks; the confirmation on `_k1` vs `_k0`. Staleness caveat: the replay's close mode uses the exact close for the price agents while the live design uses 15:45 prices — the 5-, 20- and 60-day measures move little in fifteen minutes; recorded, not simulated. Expected: the gap survives at half strength at least (+30–60% of the multiple); the live proof is the fills' slippage against the official close (target ≈ 0). Trials: 2 (+2 confirmations).

**Live design (built only on a pass; the user approved the midday PC and Claude use):** the scheduled task moves to 11:30 PT (14:30 ET): rule agents, the Claude stage (~50 min; its deadline becomes 15:35 ET instead of 09:05), then a 15:45 ET price refresh of the refresh agents (the open refresh, moved to the close), convictions, targets, the sleeve and the beta floor as now, and one slice of market-on-close orders at 15:46–15:48 (exits and entries fill together in the auction); labels scored from the signal day's close (aligned with `--exec close`); the guardian unchanged. Adoption is a schedule change: 09-22 at the earliest, after tests and a rehearsal on a state copy, one change a day (09-19 the beta floor, 09-22 close execution, 09-23 the constant confidence).
