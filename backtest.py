...
def run(days=250, refit_every=1, warmup=30, tag="", extra=(), cap=None, exec_mode="close", agents=None, end=None,
    ...
    if (config.MOMENTUM_INTRADAY if momentum_intraday is None else momentum_intraday) and exec_mode != "open":
        raise ValueError("--momentum-intraday needs --exec open: the opens frame comes from the execution prices")
    ...
    if rebalance_band is not None:
        config.REBALANCE_BAND = float(rebalance_band)                    # round 54: a held name is resized only past this share of target
    try:
