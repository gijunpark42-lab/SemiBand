...
import { BENCHMARK_START, buildComparison, type DailySeries, type EquityPoint } from "@/lib/performance";
...
export default function Performance({ equity, benchmarks, openingPrices, source, unavailable }: {
  ...
  const { dates, lines, latestDates, endDate, portfolioBaselineDate, bases } = buildComparison(equity, benchmarks, openingPrices);
  ...
  return (
    <div ref={chart} className="performance">
      <div className="performance-context">
        <span>ETF baseline: Sep 10, 2026 market open → {endDate ? `${day(endDate)} close` : "latest daily close"}</span>
        <span>{source} · free delayed data · 5-minute cache</span>
      </div>
      ...
      {endDate && latestDates.Portfolio && endDate < latestDates.Portfolio && <p className="data-notice">Comparison ends {day(endDate)} because one or more benchmarks have not published the portfolio&apos;s latest session.</p>}
      <div className="tiles" style={{ marginBottom: 16 }}>
        {Object.entries(lines).map(([name, arr]) => {
          ...
          return <div className="tile" key={name}>
            ...
            <div className="delta muted">{value == null ? "Opening baseline unavailable" : `through ${day(endDate!)} close`}</div>
      ...
      {dates.length < 2 ? <div className="empty">The comparison needs the opening baseline and at least one subsequent daily close.</div> : <>
        <div className="chart-readout"><span>{day(dates[hi])} {hi === 0 ? "open" : "close"}</span><span>{Object.entries(lines).map(([name, arr]) => `${name} ${arr[hi] == null ? "—" : pct(arr[hi]!)}`).join(" · ")}</span></div>
        <svg className="chart" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" role="img"
          aria-label={`Portfolio and benchmark returns from ${day(BENCHMARK_START)} open through ${day(endDate!)} close`}
          onPointerMove={(e) => {
          ...
          {[...new Set([0, Math.floor((dates.length - 1) / 2), dates.length - 1])].map((i) => <text key={i} className="axis" x={X(i)} y={H - 6}
            textAnchor={i === 0 ? "start" : i === dates.length - 1 ? "end" : "middle"}>{day(dates[i])}{i === 0 ? " open" : ""}</text>)}
        </svg>
        <label className="chart-scrubber">Explore dates<input type="range" min={0} max={dates.length - 1} value={hi}
          aria-label="Explore benchmark returns by date" aria-valuetext={`${day(dates[hi])} ${hi === 0 ? "open" : "close"}`} onChange={(e) => setHover(Number(e.target.value))} /></label>
      </>}
    </div>
  );
}
